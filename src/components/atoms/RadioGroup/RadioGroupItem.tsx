    ...
    return (
      <label
        className={twMerge(
          ...
          checked ? 'cursor-default' : 'hover:bg-text-1/10 active:bg-text-1/10'
        )}
        htmlFor={String(value)}
      >
        {children && <div className='py-3 pr-3'>{children}</div>}
        <div className='flex w-full items-center border-b border-border-1 py-3 pr-4'>
          <div className='flex w-full flex-col gap-1 pr-3'>
            <Typography>{label}</Typography>
            {description && (
              <Typography size='xs' color='secondary' className='break-words'>
                {description}
              </Typography>
            )}
          </div>
