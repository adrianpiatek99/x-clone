...
async fn global_timeline(db: web::Data<DbService>, req: HttpRequest) -> impl Responder {
    ...
    let post_ids: Vec<uuid::Uuid> = posts_list.iter().map(|(post, _)| post.id).collect();
    ...
    let mut media_by_post: std::collections::HashMap<uuid::Uuid, Vec<PostMedia>> = std::collections::HashMap::new();
    ...
    let likes_count_map: std::collections::HashMap<uuid::Uuid, i64> = likes_counts.into_iter().collect();
    let user_likes_set: std::collections::HashSet<uuid::Uuid> = user_likes.into_iter().collect();
