...
#[get("/following/{screen_name}")]
async fn get_following(
    db: web::Data<DbService>,
    req: HttpRequest,
    path: web::Path<String>,
    query: web::Query<GetFollowingParams>,
) -> HttpResponse {
    ...
    let mut query = follows::follows
        .inner_join(users::users.on(follows::following_id.eq(users::id)))
        .filter(follows::follower_id.eq(target_user.id))
        .into_boxed();
    ...
    let following_list = match query
        ...
        .load::<(UserSelect, Follow)>(&mut conn)
    {
        ...
        Err(_) => {
            return HttpResponse::InternalServerError().body("Error loading following");
        }
    };

    // Calculate next cursor
    let next_cursor = if following_list.len() > limit as usize {
        let next_item = &following_list[limit as usize - 1];
        Some(Cursor {
            id: next_item.0.id,
    ...
    let following_list = following_list
        .into_iter()
        .take(limit as usize)
        .collect::<Vec<_>>();
    let user_ids: Vec<Uuid> = following_list.iter().map(|(user, _)| user.id).collect();
    ...
    let response = GetFollowingResponse { users, next_cursor };
